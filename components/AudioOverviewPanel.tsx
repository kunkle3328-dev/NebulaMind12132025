...
const AudioOverviewPanel: React.FC<Props> = ({ notebook, onSaveArtifact }) => {
  ...
  const [voiceA, setVoiceA] = useState('Aoede'); // Nova default
  const [voiceB, setVoiceB] = useState('Puck'); // Atlas default
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      // Store the selected voices in the dialog object implicitly via artifact content later, 
      // or we just use them during synthesis. For now, we attach them to result for the synthesis step.
  ...
  return (
    <div className="h-full flex flex-col gap-6">
      ...
      {status === 'idle' || status === 'generating' || status === 'error' ? (
        <div className="flex flex-col items-center justify-center h-full p-4 md:p-6 animate-in fade-in overflow-y-auto">
           <div className="w-full max-w-2xl space-y-8 pb-10">
              <div className="text-center space-y-2">
                 ...
                 <p className="text-slate-400">Configure your AI podcast hosts and generation style.</p>
              </div>

              <div className="glass-panel p-6 rounded-2xl space-y-8 border border-white/10">
                 {/* TOPIC & DURATION */}
